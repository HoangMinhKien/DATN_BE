...
public class TalkHubAuthService implements ITalkHubAuthService {
    ...
    private JsonObject register(String username, String password, String deviceId, String sessionInfo){
        try{
            ...
            String query1 = "SELECT id FROM th_user WHERE username = ?";

            boolean isExisted = bridge.queryExist(query1, username);
            ...
            String query2 = "INSERT INTO th_user (username, password, role, status, createTime, refresh_token, token_exp_time) VALUE (?,?,?,?,?,?,?)";

            String hashedPassword = StringUtil.sha256(password);

            long createTime = System.currentTimeMillis();
            bridge.update(query2, username, hashedPassword, 0, 0, createTime, refreshToken, createTime+ REFRESH_TOKEN_EXPIRATION_TIME);
            long userId = bridge.queryLong(query1, username);
            JsonObject createProfile = TalkHubServices.talkHubProfileService.create(userId, username);
